...
const Pedido = ({pedido, administrador}) => {
    ...
    if(pedido.cliente !== null){
       var {nombre, apellido, telefono, email} = pedido.cliente
    }else{
         nombre, apellido,telefono, email = "Este usuario ah sido elimiando"
    }
    ...
    //funcion que modifica el color de acuerdo a su estado
    const clasePedido = () =>{
    ...
    return ( 
        <div className={`${clase} border-t-4 mt-4 bg-white rounded p-6 md:grid md:grid-cols-2 md:grap-4 shadow-lg`}>
            ...
            <div>
                ...
                {
                    administrador ? 
                    <button className="flex items-center mt-4 bg-red-800 px-5 py-2 inline-block, text-white rounded leading-tight uppercase text-xs" onClick={() => confirmarEliminarPedido()}>
                    Eliminar Pedido
